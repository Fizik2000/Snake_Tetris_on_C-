...
int is_rotation_possible(int (*field_back)[10], int (*figure)[4], int figure_x,
  ...
  while (is_row_empty(figure, 3))
    move_figure_below(figure);
  while (is_column_empty(figure, 0))
    move_figure_left(figure);
  int col = 3;
  while (is_column_empty(f, col))
    --col;
  while (figure_x + col > M - 1)
    --figure_x;
  int l = 4 < figure_y + 1 ? 4 : figure_y + 1;
  for (int i = 0; i < l; ++i) {
    for (int j = 0; j < 4; ++j) {
      if (f[i][j] && field_back[figure_y - i][figure_x + j])
        return_value = 0;
    }
  }
  return return_value;
...
void rotate(int (*field_back)[10], int (*figure)[4], int& figure_x) {
  int f[4][4];
  for (int i = 0; i < 4; ++i) {
    for (int j = 0; j < 4; ++j) {
      f[3 - j][i] = figure[i][j];
  ...
  while (is_column_empty(figure, 0))
    move_figure_left(figure);
  int col = 3;
  while (is_column_empty(figure, col))
    --col;
  while (figure_x + col > M - 1)
    {
      --figure_x;
